...
- リモートデスクトップ接続情報 (RDP) の管理
    ...
    - 名称/ホスト名での検索
    - 接続情報の複製
    - FreeRDP のオプションサポート (自動検出; 見つからない場合は `mstsc.exe` にフォールバック)
- SSH 接続情報の管理
...
リモートデスクトップの接続には、`RHarbor.exe` と同じフォルダーに対応する [FreeRDP](https://www.freerdp.com/) バイナリが見つかった場合はそれを優先して使用し、見つからない場合は Windows 標準の「リモートデスクトップ接続 (`mstsc.exe`)」 にフォールバックします。
...
RHarbor は `mstsc.exe` の代替 RDP クライアントとして [FreeRDP](https://www.freerdp.com/) をオプションでサポートしています。
